...
I've learned that no one will be crazy enough to develop an application in React,
using React raw APIs and how to use JSX, and that since is not actually a JavaScript, we need to convert
using a compiler like Babel, which, under the hood, actually generates what we wrote previously
using React raw APIs, and using JSX is much more elegant, readable and simple.
